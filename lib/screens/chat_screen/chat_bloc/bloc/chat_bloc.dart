...
// import 'package:ampify_bloc/screens/chat_screen/service/notification_service.dart';
...
//           event.chatId, event.message, event.receiverId);
...
class ChatBloc extends Bloc<ChatEvent, ChatState> {
  ...
  //     // // //  Send Push Notification to Admin after message is saved
  //     // await NotificationService.sendNotificationTo(
  //     //   receiverUID: event.receiverId, // admin UID
  ...
  Future<void> _onSendMessage(
    ...
    try {
      ...
      // 🔥 Send Push Notification to Admin via Cloud Function
      await NotificationService.sendNotificationTo(
        receiverUID: event.receiverId, // admin UID
        title: "New Message from ${event.message.senderName}",
        body: event.message.text,
      );
    } catch (e) {
      emit(ChatError('Failed to send message'));
